        ...
        if message.channel.id == 700842187058970624: # Channel id of "inventory-editor"
            if message.content.startswith('removecharacter'):
                ...
                if character_id in self.characterinventory:
                    await channel.purge(limit=10)
                    msg2 = await channel.send(f"Are you sure you want to delete the character with ID [**{character_id}**]?\n**yes** or **no**")
                    ...
                    if response == "yes":                        
                        ...
                        member = guild.get_member(character_id_int)
                        member_nick = member.nick

                        users_role = discord.utils.get(guild.roles, name=member_nick)
                        await users_role.delete()

                        # await guild.delete_role(698522830525956097, users_role)
